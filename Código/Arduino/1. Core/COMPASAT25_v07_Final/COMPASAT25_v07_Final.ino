...
// BMP280
Adafruit_BMP280 bmp;
float presionAlNivelDelMar = 1004.8; // Presión en hPa según calibración a nivel del mar
...
void loop() {
  ...
  if (!motorAbierto && altitud < (alturaMaxima - 5)) {
    ...
    if (logFile.open("COMPASAT.txt", O_WRITE | O_APPEND)) {
      logFile.println("Motor abierto");
      logFile.close();
    }
  }

    // Motor: cerrar si faltan 50 metros para el suelo
  if (motorAbierto && altitud <= (alturaInicial + 50)) {
    moveStepper(stepsPer90, false);
    motorAbierto = false;
    Serial.println("Motor cerrado");

    if (logFile.open("COMPASAT.txt", O_WRITE | O_APPEND)) {
      logFile.println("Motor cerrado");
      logFile.close();
    }
  }
